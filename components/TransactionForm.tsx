...
const TransactionForm: React.FC<Props> = ({ onClose }) => {
  ...
  // Load defaults if provided (e.g. from "Pay Bill" button or Edit Tap)
  useEffect(() => {
      if (transactionModalDefault) {
          // Identify if this is an Edit operation (existing ID)
          if (transactionModalDefault.id) {
              setEditingId(transactionModalDefault.id);
          }
          ...
          // Category Initialization logic for Subcategories
          ...
          // Smart Defaults for normal open
  ...
  useEffect(() => {
    ...
    if (type === 'transfer') {
        ...
        // Auto-select valid To Account if not set
    ...
    // When type switches, reset view to root and pick default category
    setCategoryViewMode('root');
    setActiveParentCategory(null);

    // Only auto-switch category if we are NOT in editing mode or if the type changed significantly
  ...
  const handleCategoryClick = (cat: Category) => {
      ...
      if (subs.length > 0) {
          ...
          // Select and stay (or if in sub mode, select child)
  ...
  // Check if this is a Bill Payment (Transfer to Credit Card)
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex justify-center items-end sm:items-center pointer-events-none">
        {/* Backdrop for desktop centering effect */}
        ...
        <div className="w-full max-w-md h-[100dvh] bg-gray-50 dark:bg-[#0c0a18] text-zinc-900 dark:text-white pointer-events-auto flex flex-col animate-in slide-in-from-bottom-10 duration-200 shadow-2xl overflow-hidden relative border-x border-zinc-200 dark:border-white/5">
            ...
            <div className="flex justify-between items-center px-6 py-4 bg-white dark:bg-[#0c0a18] shrink-0 border-b border-zinc-100 dark:border-white/5">
                 ...
                 <div className="flex items-center gap-2 -mr-2">
                     {editingId && (
                         showDeleteConfirm ? (
                             <div className="flex items-center gap-2 animate-in slide-in-from-right-4 fade-in duration-200 mr-2">
                                <span className="text-xs font-bold text-red-500 mr-1">Delete?</span>
                                <button 
                                    onClick={() => setShowDeleteConfirm(false)} 
                                    className="p-1.5 px-3 rounded-full bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 text-xs font-bold hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors"
                                >
                                    Cancel
                                </button>
                                <button 
                                    onClick={confirmDelete} 
                                    className="p-1.5 px-3 rounded-full bg-red-500 text-white text-xs font-bold hover:bg-red-600 transition-colors shadow-sm"
                                >
                                    Confirm
                                </button>
                             </div>
                         ) : (
                             <button onClick={() => setShowDeleteConfirm(true)} className="p-2 rounded-full bg-red-50 dark:bg-red-900/10 text-red-500 hover:bg-red-100 dark:hover:bg-red-900/20 transition-colors">
                                 <Trash2 size={20} />
                             </button>
                         )
                     )}
                     <button onClick={onClose} className="p-2 rounded-full bg-zinc-100 dark:bg-white/5 text-zinc-500 hover:text-zinc-900 dark:hover:text-white transition-colors">
                        <X size={24} />
                     </button>
                 </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar px-6 pb-32">
                ...
                <div className="space-y-8">
                    ...
                    {/* Category Section (Nested Support via Drill-down) */}
                    {type !== 'transfer' && (
                        <div className="space-y-3">
                            <div className="flex justify-between items-center px-1">
                                <label className="text-xs font-bold text-zinc-500 uppercase tracking-wider flex items-center gap-2">
                                    <Tag size={14} /> Category {categoryViewMode === 'sub' && activeParentCategory && <span className="text-violet-600 dark:text-violet-400">/ {activeParentCategory.name}</span>}
                                </label>
                            </div>
                            
                            <div className="grid grid-cols-4 gap-2 animate-in fade-in slide-in-from-right-4 duration-300" key={categoryViewMode}>
                                {/* Root View: Show Parents */}
                                {categoryViewMode === 'root' && rootCategories.map(cat => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold mb-1 shadow-sm relative ${cat.color}`}>
                                                <CategoryIcon iconName={cat.icon} size={16} />
                                            </div>
                                            <div className="w-full text-center flex flex-col items-center justify-center">
                                                <span className={`text-[10px] font-bold truncate w-full text-center leading-tight ${isFamilySelected ? 'text-zinc-900 dark:text-white' : 'text-zinc-500'}`}>
                                                    {cat.name}
                                                </span>
                                            </div>
                                        </button>
                                ...
                                {/* Sub View: Show Back Button + Parent (General) + Children */}
                                {categoryViewMode === 'sub' && activeParentCategory && (
                                    <>
                                        {/* 1. Back Button */}
                                        <button
                                            onClick={handleBackToRoot}
                                            className="flex flex-col items-center p-2 rounded-2xl transition-all border glass-panel border-transparent hover:bg-black/5 dark:hover:bg-white/5 relative overflow-hidden aspect-square justify-center"
                                        >
                                        ...
                                        {/* 2. The Parent itself (General Option) */}
                                        <button
                                        ...
                                        >
                                            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold mb-1 shadow-sm relative ${activeParentCategory.color}`}>
                                                <CategoryIcon iconName={activeParentCategory.icon} size={16} />
                                            </div>
                                            <div className="w-full text-center flex flex-col items-center justify-center">
                                                <span className={`text-[10px] font-bold truncate w-full text-center leading-tight ${selectedCategoryId === activeParentCategory.id ? 'text-zinc-900 dark:text-white' : 'text-zinc-500'}`}>
                                        ...
                                        {getSubcategories(activeParentCategory.id).map(sub => (
                                            ...
                                            >
                                                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold mb-1 shadow-sm relative scale-90 ${sub.color}`}>
                                                    <CategoryIcon iconName={sub.icon} size={14} />
                                                </div>
                                                <div className="w-full text-center flex flex-col items-center justify-center">
                                                    <span className={`text-[10px] font-bold truncate w-full text-center leading-tight ${selectedCategoryId === sub.id ? 'text-zinc-900 dark:text-white' : 'text-zinc-500'}`}>
                                                        {sub.name}
                                                    </span>
                                                </div>
                                            </button>
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="glass-panel rounded-2xl overflow-hidden divide-y divide-zinc-100 dark:divide-white/5">
                            ...
                            {/* Date Picker - Ensure text-base to prevent zoom */}
                            <div className="flex items-center p-4 hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
                            ...
                            {/* Note Input - Ensure text-base to prevent zoom */}
                            <div className="flex items-center p-4 hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
                                <div className="w-8 flex justify-center text-zinc-400">
                                    <FileText size={18} />
                                </div>
                                <input 
                                    ...
                                    placeholder={isPayBill ? "Credit Card Payment" : "Add a note..."}
                                         ...
                                         {(['daily', 'weekly', 'monthly'] as RecurrenceRule[]).map(opt => (
                                             <button
                                                ...
                                                className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-all capitalize border ${
                                                    ...
                                                    : 'bg-black/5 dark:bg-white/5 text-zinc-500 border-transparent hover:bg-black/10 dark:hover:bg-white/10'
                                                }`}
                                             >
                                                 {opt}
                                             </button>
            ...
            <div className="absolute bottom-0 left-0 w-full p-4 bg-white/80 dark:bg-[#0c0a18]/80 backdrop-blur-xl border-t border-zinc-200 dark:border-white/5 pb-safe z-10">
                 ...
                 >
                    ...
                    {editingId ? 'Update Transaction' : (isPayBill ? 'Confirm Payment' : 'Save Transaction')}
                 </button>
            </div>
        </div>
    </div>,
